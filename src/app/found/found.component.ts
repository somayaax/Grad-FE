...
export class FoundComponent implements OnInit {
  ...
  found(postInfo: any) {
    ...
    formData.append('title', this.foundForm.get('title')?.value)
    formData.append('desc', this.foundForm.get('desc')?.value)
    formData.append('location', this.foundForm.get('location')?.value)
    postInfo = formData
    this._postsService.addFound(postInfo).subscribe((res) => {
      if (res.message === "done") {
        this.submitted = true
        console.log(res);
